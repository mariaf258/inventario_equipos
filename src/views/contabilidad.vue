...
const obtenerDatos = async () => {
  ...
  const empleadosContabilidad: Equipo[] = empleados.filter((empleado) => /^mla-ct-\d+$/i
...
onMounted(() => {
  obtenerDatos().then(() => {
    filteredEmpleado.value = [...empleadosModuloContabilidad.value];
  });
});
...
const filtrarEmpleados = (event: Event) => {
  ...
  filteredEmpleado.value = empleadosModuloContabilidad.value.filter((empleado) =>
...
const guardarModulo =()=>{
  localStorage.setItem('modulo',  "ct")
}
