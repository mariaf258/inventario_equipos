...
let empleadosModuloJuridico =ref<Equipo[]>([]);
const obtenerDatos = async () => {
  const empleados:Equipo[] = await empleadoServicio.obtenerEmpleados()
  console.log(empleados);
  
  const empleadosJuridico: Equipo[] = empleados.filter((empleado) => /^mla-ju-\d+$/i
...
.map((empleado) => ({
  ...
  console.log(empleadosJuridico);
  empleadosModuloJuridico.value = empleadosJuridico;
  console.log({empleadosModuloJuridico})
...
onMounted(() => {
  obtenerDatos().then(() => {
    filteredEmpleado.value = [...empleadosModuloJuridico.value];
  });
});
...
const filtrarEmpleados = (event: Event) => {
  ...
  filteredEmpleado.value = empleadosModuloJuridico.value.filter((empleado) =>
...
const guardarModulo =()=>{
  localStorage.setItem('modulo',  "ju")
}
  ...
  <div id="app1">
    ...
    <div class="main-content-group">
      <div class="main-content">
        ...
        <div class="departamento"><h1>JURIDICO</h1></div>
