...
let empleadosModuloComercial =ref<Equipo[]>([]);
const obtenerDatos = async () => {
  const empleados:Equipo[] = await empleadoServicio.obtenerEmpleados()
  console.log(empleados);
  
  const empleadosComercial: Equipo[] = empleados.filter((empleado) => /^mla-cm-\d+$/i
...
.map((empleado) => ({
  ...
  console.log(empleadosComercial);
  empleadosModuloComercial.value = empleadosComercial;
  console.log({empleadosModuloComercial})
...
onMounted(() => {
  obtenerDatos().then(() => {
    filteredEmpleado.value = [...empleadosModuloComercial.value];
  });
});
...
const filtrarEmpleados = (event: Event) => {
  ...
  filteredEmpleado.value = empleadosModuloComercial.value.filter((empleado) =>
...
const guardarModulo =()=>{
  localStorage.setItem('modulo',  "cm")
}
