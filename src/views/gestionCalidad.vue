...
let empleadosModuloGestionCalidad =ref<Equipo[]>([]);
const obtenerDatos = async () => {
  const empleados:Equipo[] = await empleadoServicio.obtenerEmpleados()
  console.log(empleados);
  
  const empleadosGestionCalidad: Equipo[] = empleados.filter((empleado) => /^mla-gc-\d+$/i
...
.map((empleado) => ({
  ...
  console.log(empleadosGestionCalidad);
  empleadosModuloGestionCalidad.value = empleadosGestionCalidad;
  console.log({empleadosModuloGestionCalidad})
...
onMounted(() => {
  obtenerDatos().then(() => {
    filteredEmpleado.value = [...empleadosModuloGestionCalidad.value];
  });
});
...
const filtrarEmpleados = (event: Event) => {
  ...
  filteredEmpleado.value = empleadosModuloGestionCalidad.value.filter((empleado) =>
  ...
  <div id="app1">
    ...
    <div class="main-content-group">
      <div class="main-content">
        ...
        <div class="departamento"><h1>GESTION DE CALIDAD</h1></div>
