...
const obtenerDatos = async () => {
  ...
  const empleadosAmbiental: Equipo[] = empleados.filter((empleado) => /^mla-am-\d+$/i
...
onMounted(() => {
  obtenerDatos().then(() => {
    filteredEmpleado.value = [...empleadosModuloAmbiental.value];
  });
});
...
const filtrarEmpleados = (event: Event) => {
  ...
  filteredEmpleado.value = empleadosModuloAmbiental.value.filter((empleado) =>
