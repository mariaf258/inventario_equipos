...
const obtenerDatos = async () => {
  ...
  const empleadosArchivo: Equipo[] = empleados.filter((empleado) => /^mla-ar-\d+$/i
...
onMounted(() => {
  obtenerDatos().then(() => {
    filteredEmpleado.value = [...empleadosModuloArchivo.value];
  });
});
...
const filtrarEmpleados = (event: Event) => {
  ...
  filteredEmpleado.value = empleadosModuloArchivo.value.filter((empleado) =>
...
const guardarModulo =()=>{
  localStorage.setItem('modulo',  "ar")
}
