...
const obtenerDatos = async () => {
  ...
  const empleadosSistemas: Equipo[] = empleados.filter((empleado) => /^mla-si-\d+$/i
...
onMounted(() => {
  obtenerDatos().then(() => {
    filteredEmpleado.value = [...empleadosModuloSistemas.value];
  });
});
...
const filtrarEmpleados = (event: Event) => {
  ...
  filteredEmpleado.value = empleadosModuloSistemas.value.filter((empleado) =>
