...
const obtenerDatos = async () => {
  ...
  const empleadosGerencia: Equipo[] = empleados.filter((empleado) => /^mla-ge-\d+$/i
...
onMounted(() => {
  obtenerDatos().then(() => {
    filteredEmpleado.value = [...empleadosModuloGerencia.value];
  });
});
...
const filtrarEmpleados = (event: Event) => {
  ...
  filteredEmpleado.value = empleadosModuloGerencia.value.filter((empleado) =>
