...
const obtenerDatos = async () => {
  ...
  const empleadosAdministracion: Equipo[] = empleados.filter((empleado) => /^mla-ad-\d+$/i
...
.map((empleado) => ({
  ...
  console.log(empleadosAdministracion);
  empleadosModuloAdministracion.value = empleadosAdministracion;
  console.log({empleadosModuloAdministracion})
...
onMounted(() => {
  obtenerDatos().then(() => {
    filteredEmpleado.value = [...empleadosModuloAdministracion.value];
  });
});
...
const filtrarEmpleados = (event: Event) => {
  ...
  filteredEmpleado.value = empleadosModuloAdministracion.value.filter((empleado) =>
...
const guardarModulo =()=>{
  localStorage.setItem('modulo',  "ad")
}
  ...
  <div id="app1">
    ...
    <div class="main-content-group">
      <div class="main-content">
        ...
        <div class="departamento"><h1>ADMINISTRACION</h1></div>
